...
ItemChair::ItemChair(const Vector3& _pos, const Vector3& _size, const Tag& _objectTag, const SceneBase::Scene _sceneTag)
	: ItemBase(_sceneTag, _objectTag)
{
	...
	mMeshComponent->SetMesh(RENDERER->GetMesh("Assets/Model/Item/Chair.gpmesh"));
...
void ItemChair::UpdateGameObject(float _deltaTime)
{
	// フラグが上がっていたら
...
void ItemChair::OnCollision(const GameObject& _hitObject)
{
	// 当たったのでフラグを上げる
	mCollisionFlag = true;
